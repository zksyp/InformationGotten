...
import com.zksyp.informationgotten.bean.response.BaseResponse;
...
//
    ...
    public static <T> Func1<T, Boolean> verifyBusiness(ErrorVerify errorVerify) {
        return response -> {
            if (response instanceof BaseResponse) {
                BaseResponse baseResponse = (BaseResponse) response;
                ResponseCode responseCode = ResponseCode.toEnum(baseResponse.resultCode);
                boolean isSuccess = responseCode == ResponseCode.RESULT_OK;
                if (!isSuccess) {
                    if (errorVerify != null) {
                        LogUtil.e("返回错误码：" + baseResponse.resultCode + "\t\t\t错误信息：" + baseResponse.errorDesc);
                        errorVerify.call(baseResponse.resultCode + "", baseResponse.errorDesc);
                    }
                }
                return isSuccess;
            } else {
